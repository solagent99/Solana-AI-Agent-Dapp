...
const compressData = async (data: any): Promise<Buffer> => {
  const jsonStr = JSON.stringify(data);
  return gzipAsync(Buffer.from(jsonStr));
};

const decompressData = async (buffer: Buffer): Promise<any> => {
...
jest.mock('ioredis', () => {
  ...
  const mockRedis = {
    ...
    get: jest.fn(async (key: RedisKey) => {
      const value = mockStore.get(key.toString());
      return value || null;
    }),
    set: jest.fn(async (key: RedisKey, value: RedisValue) => {
  ...
  return jest.fn().mockImplementation(() => mockRedis as unknown as Redis);
});
