...
function validateEnvironment() {
    const missingVars = [];
    const requiredVars = ["GROQ_API_KEY", "RPC_URL", "SOLANA_PRIVATE_KEY"];
...
async function initializeAgent() {
    try {
        ...
        const solanaAgentKitInstance = new SolanaAgentKit(process.env.SOLANA_PRIVATE_KEY, process.env.RPC_URL, process.env.GROQ_API_KEY);
        const tools = createSolanaTools(solanaAgentKitInstance);
...
async function configureMarketMaker() {
...
async function runMarketMakerMode(agent, config) {
    try {
        ...
        Important! Only send 1 transaction, buy and sells can be combined into a single solana_batch_order.`;
                ...
                await new Promise((resolve) => setTimeout(resolve, marketMakerConfig.intervalSeconds * 1000));
            }
            catch (error) {
                if (error instanceof Error) {
